...
class S2SModel(object):
    def __init__(self,
        ...
        # LSTM cells
        encoder_cell = tf.contrib.rnn.GRUCell(size)
        encoder_cell = tf.contrib.rnn.DropoutWrapper(encoder_cell, output_keep_prob=dropout)
        encoder_cell = tf.contrib.rnn.MultiRNNCell([encoder_cell] * 8)
        # Gstacked_rnn = []
        # Gstacked_bw_rnn = []
        # for i in range(3):
        #     Gstacked_rnn.append(tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(size), output_keep_prob=dropout))
        #     Gstacked_bw_rnn.append(tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(size), output_keep_prob=dropout))
        #
        # # 建立前向和后向的三层RNN
        # encoder_cell = tf.contrib.rnn.MultiRNNCell(Gstacked_rnn)
        # encoder_cell_bw = tf.contrib.rnn.MultiRNNCell(Gstacked_bw_rnn)

        decoder_cell = tf.contrib.rnn.GRUCell(size)
        decoder_cell = tf.contrib.rnn.DropoutWrapper(decoder_cell, output_keep_prob=dropout)
        decoder_cell = tf.contrib.rnn.MultiRNNCell([decoder_cell] * 4)
        ...
        def embedding_attention_seq2seq(encoder_inputs,
            ...
            with variable_scope.variable_scope(
                ...
                #     encoder_cell, encoder_cell_bw, encoder_inputs, sequence_length=len(encoder_inputs), dtype=dtype)
        ...
        def seq2seq_f(encoder_inputs, decoder_inputs, do_decode):
            ...
            tmp_encoder_cell = copy.deepcopy(encoder_cell)
            # tmp_encoder_cell_bw = copy.deepcopy(encoder_cell_bw)
            tmp_decoder_cell = copy.deepcopy(decoder_cell)
            ...
            return embedding_attention_seq2seq(
                ...
                tmp_encoder_cell,  # 自定义的cell,可以是GRU/LSTM, 设置multilayer等
                # tmp_encoder_cell_bw,  # 自定义的cell,可以是GRU/LSTM, 设置multilayer等
                tmp_decoder_cell,  # 自定义的cell,可以是GRU/LSTM, 设置multilayer等
