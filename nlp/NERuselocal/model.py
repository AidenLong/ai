            ...
            for j in range(self.repeat_times):
                for i in range(len(self.layers)):
                    ...
                    with tf.variable_scope("atrous-conv-layer-%d" % i,
                        ...
                        # filters
                        # 相当于CNN中的卷积核，要求是一个4维Tensor，具有[filter_height, filter_width, channels,
                        # out_channels]这样的shape，具体含义是[卷积核的高度，卷积核的宽度，图像通道数，卷积核个数]，
                        # 同理这里第三维channels，就是参数value的第四维
                        # rate
                        # 要求是一个int型的正数，正常的卷积操作应该会有stride（即卷积核的滑动步长），
                        #   但是空洞卷积是没有stride参数的，这一点尤其要注意。取而代之，
                        #   它使用了新的rate参数，那么rate参数有什么用呢？它定义为我们在输入图像上卷积时的采样间隔，
                        ...
                        # 这个函数已经默认了stride=1，也就是滑动步长无法改变，固定为1。
