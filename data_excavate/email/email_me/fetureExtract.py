...
def get_email_address(strl):  # 发送接收地址提取
...
df['from_address'] = pd.Series(map(lambda str: get_email_address(str), df['from']))  # map映射并添加
df['to_address'] = pd.Series(map(lambda str: get_email_address(str), df['to']))
print("========to address=======================")
...
# 数据转换
date_time_extract_result = list(map(lambda st: date_format(st), df['date']))
...
df['content_length_type'] = pd.Series(map(lambda st: get_len(st), df['content_length']))
...
a = np.arange(1, 20000)
plt.plot(a, list(map(lambda t: len_info_exec(t), a)), label=u'信息量')
# plt.plot(df['content_length'], list(map(lambda t: 长度信息量计算(t) ,df['content_length'])), label = u'信息量')
plt.grid(True)
plt.legend(loc=0)
plt.show()
# sys.exit(0)
df['content_length_sema'] = list(map(lambda st: len_info_exec(st), df['content_length']))
